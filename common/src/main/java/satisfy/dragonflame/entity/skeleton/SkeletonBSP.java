package satisfy.dragonflame.entity.skeleton;
 ...
 */
public class SkeletonBSP extends Skeleton implements SmartBrainOwner<SkeletonBSP> {
    public SkeletonBSP(EntityType<? extends SkeletonBSP> entityType, Level level) {
    ...
    @Override
    public List<ExtendedSensor<SkeletonBSP>> getSensors() {
        return ObjectArrayList.of(
                new NearbyPlayersSensor<>(), 							// Keep track of nearby players
                new NearbyLivingEntitySensor<SkeletonBSP>()
    ...
    @Override
    public BrainActivityGroup<SkeletonBSP> getCoreTasks() {
        return BrainActivityGroup.coreTasks(
                ...
                new StrafeTarget<>().stopStrafingWhen(entity -> !isHoldingBow(entity)).startCondition(SkeletonBSP::isHoldingBow),	// Strafe around target
                new MoveToWalkTarget<>());																					// Move to the current walk target
    }

    @Override
    public BrainActivityGroup<SkeletonBSP> getIdleTasks() {
        return BrainActivityGroup.idleTasks(
                new FirstApplicableBehaviour<SkeletonBSP>( 				// Run only one of the below behaviours, trying each one in order. Include explicit generic typing because javac is silly
    ...
    @Override
    public BrainActivityGroup<SkeletonBSP> getFightTasks() {
        return BrainActivityGroup.fightTasks(
                ...
                new FirstApplicableBehaviour<>( 																							  	 // Run only one of the below behaviours, trying each one in order
                        new BowAttack<SkeletonBSP>(20).startCondition(SkeletonBSP::isHoldingBow),	 												 // Fire a bow, if holding one
