...
public class ShatterbrandSwordItem extends SwordItem implements EnchantingBehavior {
    ...
    public ShatterbrandSwordItem(Tier toolMaterial, Properties properties) {
        super(toolMaterial, 0, -0F, properties);
    }
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (result && !target.level().isClientSide() && !target.fireImmune()) {
            ...
            for (int var1 = 0; var1 < 20; ++var1) {
                ...
                target.level().addParticle(ParticleTypes.ASH, px, py, pz, 0.02, 0.02, 0.02);
                target.level().addParticle(ParticleTypes.WHITE_ASH, px, py, pz, 0.02, 0.02, 0.02);
            }
        }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, @NotNull List<Component> tooltip, TooltipFlag context) {
        tooltip.add(Component.translatable("lore.dragonflame.shatterbrand1").withStyle(ChatFormatting.GOLD, ChatFormatting.ITALIC));
        tooltip.add(Component.empty());
        tooltip.add(Component.translatable("lore.dragonflame.shatterbrand2").withStyle(ChatFormatting.WHITE, ChatFormatting.OBFUSCATED));
